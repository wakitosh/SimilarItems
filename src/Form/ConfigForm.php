  ...
  public function init(): void {
    ...
    $this
      ->add([
        ...
        'options' => [
          'label' => 'タイトルルール',
          'info' => 'ベースタイトル抽出に用いる区切り文字を指定します。区切り文字は「指定した文字列の完全一致」で判定します（前後スペースも含む）。区切り文字が設定されている場合、それ以外の自動的な末尾の数字/巻号除去は行いません。',
        ],
      ])
      ->add([
      ...
      ->add([
        ...
        'options' => [
          ...
          'label' => 'タイトルと巻号の区切り文字',
                  // @translate
          'info' => 'ベースタイトルを判定するために、タイトルと巻号を区切る文字列を1行に1つ指定します。判定は完全一致です（前後のスペースも含みます）。例: 「 , 」を指定した場合、「, 」では一致しません。区切り文字が設定されている場合、区切りに一致したときだけ右側を切り落とし、それ以外の自動除去（末尾の数字/年号/巻号など）は行いません。',
        ],
        'attributes' => [
    ...
    $this
      ...
      ->add([
        ...
        'options' => [
          ...
          // @translate
          'info' => '職員は資料を発見するためではなく、内容や表示を確認するために閲覧します。行動が一般利用者と異なるうえ、動作確認のクリックもそのまま記録されてしまうため、既定では記録しません。職員自身の利用を分析対象にする場合のみ外してください（その場合は「ログイン中のユーザ ID も記録する」も併せてご検討ください。無効のままだと、後からどれが職員の記録か判別できません）。',
        ],
        'attributes' => [
